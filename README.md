...
* **High-Performance Multithreading:** Built on `numpy`, `pandas`, and standard library `ThreadPoolExecutor` to execute pairwise comparisons across all available CPU cores, supporting large  resampling iterations (e.g., $> 4,000$) in seconds (depends on sample size).
